        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl
                v-for="(skuSaleAttr, index) in skuSaleAttrValueList"
                :key="skuSaleAttr.id"
              >
                ...
                <dd
                  ...
                  :key="spuSaleAttr.id"
                  @click="
                    changeActive(spuSaleAttr, skuSaleAttr.spuSaleAttrValueList)
                  "
                >
...
export default {
  ...
  computed: {
    ...
    skuImageList() {
      ...
      // skuInfo.skuImageList(空数组里的空对象) ----> [{}]   防止值为undefined报错
    },
  },
  //产品售卖属性值切换高亮
  methods: {
    changeActive(spuSaleAttr, arr) {
      //遍历全部的售卖属性值 让所有的都没有高亮isChecked = 0
      arr.forEach((item) => {
        item.isChecked = 0;
      });
      //点击的那个有
    ...
    async addShopCart() {
      try {
        ...
        //路由跳转
        //简单的数据 skuNumber ----> 通过query给路由传递
        this.$router.push({name:'addcartsuccess',query:{skuNum:this.skuNumber}})
        
        //复杂的skuInfo  ----> 通过会话存储(会话结束数据消失)  会话存储存储字符串(字符串传过去在变回对象)
